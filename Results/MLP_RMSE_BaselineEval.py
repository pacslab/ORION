# Tables 4 - 5 Results Evaluation Code
import os, re, sys
import numpy as np
import pandas as pd
...
# ID / textual columns that should never be used as features
ID_TXT_COLS_DEFAULT = {
    "gpu_name",
    "dataset",
    "input_shape",
...
# Categorical columns that will be one-hot encoded (dummies),
# while the raw base columns themselves are excluded as features.
...
def add_hardware_flags(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "gpu_name" in df.columns:
        ...
        df["has_tensor_cores"] = df["gpu_name"].apply(
            lambda s: int(_contains_any(s, ["a100", "a40", "a10", "l40", "l4", "h100", "rtx 50", "rtx 5090", "v100", "t4"]))
        ).astype(int)
        ...
        df["is_datacenter"] = df["gpu_name"].apply(
            lambda s: int(_contains_any(s, ["a100", "h100", "v100", "a40", "l40", "l4", "a10", "t4", "p100", "p4"]))
        ).astype(int)
    else:
...
def add_cnn_computational_feature(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    if "flops_train_per_sample" in df.columns:
        df["F_CNN_sample"] = df["flops_train_per_sample"].astype(float)
    elif "flops_fwd_per_sample" in df.columns:
        df["F_CNN_sample"] = df["flops_fwd_per_sample"].astype(float) * 3.0
    elif "flops_train_per_batch" in df.columns and "batch_size" in df.columns:
        df["F_CNN_sample"] = df["flops_train_per_batch"].astype(float) / np.maximum(
            1, df["batch_size"].astype(float)
        )
    else:
        df["F_CNN_sample"] = np.nan

    if "flops_train_per_batch" in df.columns:
        df["F_CNN_batch"] = df["flops_train_per_batch"].astype(float)
    else:
        df["F_CNN_batch"] = df["F_CNN_sample"] * df.get("batch_size", 1).astype(float)

    df["log_F_CNN_sample"] = np.log1p(df["F_CNN_sample"].astype(float))
    df["log_F_CNN_batch"] = np.log1p(df["F_CNN_batch"].astype(float))
    return df
...
def kfold_ORION_vs_prenet(df: pd.DataFrame, n_splits: int = 5) -> pd.DataFrame:
    ...
    per (model, vcpu, storage), plus % improvement of ORION.
    """

    # Feature engineering
    df = engineer_gpu_model_features(df)
    df = add_hardware_flags(df)
    df = add_cnn_computational_feature(df)
    ...
    #   - ORION: target avg_batch_time_ms
    #   - PreNeT: target T_gpu_ms
    X_ORION, y_ORION, feats_ORION = build_feature_matrix(
        df, TARGET_ORION, include_model_oh=INCLUDE_MODEL_OH, include_opt_oh=INCLUDE_OPT_OH
    )
    X_prenet, y_prenet, feats_prenet = build_feature_matrix(
        df, TARGET_PRENET, include_model_oh=INCLUDE_MODEL_OH, include_opt_oh=INCLUDE_OPT_OH
    )

    # Make sure feature sets are identical (they should be)
    if set(feats_ORION) != set(feats_prenet):
        raise ValueError("Feature sets for ORION and PreNeT differ; check LEAKY_COLS and build_feature_matrix.")
    ...
    for train_idx, test_idx in kf.split(X_ORION):
        ...
        # PreNeT: train on T_gpu_ms, eval vs avg_batch_time_ms
        ...
        # These predictions are compute-only; we compare them to avg_batch_time_ms
        preds_prenet[test_idx] = yhat_pre_gpu_ms

    # Build group-wise RMSE per (model, vcpu, storage)
    models = df["model"].astype(str)
    vcpus = df["vcpu"].astype(int)
    storage = df["storage"].astype(str)

    rows = []
    for (m, v, s) in sorted(set(zip(models, vcpus, storage))):
        mask = (models == m) & (vcpus == v) & (storage == s)
        if mask.sum() < 2:
        ...
        # % improvement of ORION vs PreNeT (positive = ORION better)
        if rmse_p > 0:
    ...
    result = pd.DataFrame(rows).sort_values(["model", "vcpu", "storage"]).reset_index(drop=True)
    return result
...
def main():
    ...
    if summary.empty:
        print("No groups with sufficient samples for RMSE computation.")
    else:
        # Format RMSE & improvement with two decimals for printing
        summary_fmt = summary.copy()
        for col in ["RMSE_ORION_ms", "RMSE_PreNeT_ms", "Improvement_ORION_vs_PreNeT_%"]:
            summary_fmt[col] = summary_fmt[col].map(
                lambda x: f"{x:.2f}" if pd.notnull(x) else "nan"
            )

        with pd.option_context("display.max_rows", None, "display.max_columns", None):
            print("\n=== RMSE per (model, vcpu, storage) ===")
