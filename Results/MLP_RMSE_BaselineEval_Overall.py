...
def add_hardware_flags(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "gpu_name" in df.columns:
        ...
        df["has_tensor_cores"] = df["gpu_name"].apply(
            lambda s: int(_contains_any(s, ["a100", "a40", "a10", "l40", "l4",
                                            "h100", "rtx 50", "rtx 5090", "v100", "t4"]))
        ).astype(int)
        ...
        df["is_datacenter"] = df["gpu_name"].apply(
            lambda s: int(_contains_any(s, ["a100", "h100", "v100", "a40",
                                            "l40", "l4", "a10", "t4", "p100", "p4"]))
        ).astype(int)
    else:
...
def kfold_ORION_vs_prenet(df: pd.DataFrame, n_splits: int = 5) -> pd.DataFrame:
    ...
    per model (aggregated across vcpu and storage),
    plus % improvement of ORION.
    """
...
def main():
    ...
    if summary.empty:
        ...
        with pd.option_context("display.max_rows", None, "display.max_columns", None):
            print("\n=== MLP RMSE per model (aggregated across vCPU & storage) ===")
