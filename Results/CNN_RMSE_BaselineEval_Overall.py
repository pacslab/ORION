...
def kfold_ORION_vs_prenet(df: pd.DataFrame, n_splits: int = 5) -> pd.DataFrame:
    ...
    ORION uses all engineered features (including vcpu / storage).
    PreNeT uses the same engineering pipeline but explicitly
    excludes vcpu and all storage-related features from its inputs.
    """
    ...
    # Build RMSE per model (aggregate across vcpu and storage)
...
def main():
    ...
    if summary.empty:
        ...
        with pd.option_context("display.max_rows", None, "display.max_columns", None):
            print("\n=== CNN RMSE per model (aggregated across vCPU & storage) ===")
