...
def auto_log1p_heavy_tails(
    df: pd.DataFrame, target: str, extra_drop: set
...
def kfold_ORION_vs_prenet(df: pd.DataFrame, n_splits: int = 5) -> pd.DataFrame:
    ...
    #   - ORION: target avg_batch_time_ms (uses all features)
    #   - PreNeT: target T_gpu_ms (GPU-only, no vcpu/storage-related features)
    ...
    # For PreNeT: explicitly drop any vcpu or storage-related features
    pre_drop_cols = [
        c
        for c in X_prenet.columns
        if ("vcpu" in c.lower()) or c.lower().startswith("storage_")
    ]
    if pre_drop_cols:
        X_prenet = X_prenet.drop(columns=pre_drop_cols)
...
def main():
    ...
    if summary.empty:
        ...
        with pd.option_context("display.max_rows", None, "display.max_columns", None):
            print("\n=== CNN RMSE per (model, vcpu, storage) ===")
