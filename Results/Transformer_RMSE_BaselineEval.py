...
def add_hardware_flags(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "gpu_name" in df.columns:
        # If arch_family column already exists in CSV, keep it but also
        # build a derived version from gpu_name for consistency.
...
def add_transformer_computational_feature(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Effective token length: prefer seq_len (vision/text encoder-only),
    # then tgt_len (seq2seq), then src_len
...
def add_batch_aware_hardware_interactions(df: pd.DataFrame) -> pd.DataFrame:
    """
    Batch-size and hardware interaction features; uses what exists
    in Transformer.csv and falls back safely when columns are missing.
    """
    df = df.copy()
...
def fit_predict_with_scaling(Xtr: pd.DataFrame, ytr: np.ndarray, Xte: pd.DataFrame):
    ...
    #   - drop it if training column is all-NaN
    #   - or fill NaNs in train/test with the TRAIN median
    drop_cols = []
...
def kfold_ORION_vs_prenet(df: pd.DataFrame, n_splits: int = 5) -> pd.DataFrame:
    ...
    Specialized to the Transformer benchmark CSV.
...
def main():
    ...
    if summary.empty:
        ...
        with pd.option_context("display.max_rows", None, "display.max_columns", None):
            print("\n=== Transformer: RMSE per (model, vcpu, storage) ===")
